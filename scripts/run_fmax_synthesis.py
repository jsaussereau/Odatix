  ...
  for i_chunk in range(nb_chunks):
    ...
    for i_arch in architecture_instances_chunk:
      ...
      if i_arch.use_parameters:
        ...
        replace_params(base_text_file=param_target_file, replacement_text_file=param_filename, output_file=param_target_file, start_delimiter=i_arch.start_delimiter, stop_delimiter=i_arch.stop_delimiter, replace_all_occurrences=False, silent=True)
        # print()
      ...
      process = run_parallel(
        command="make -f {} {}".format(tool_makefile_file, synth_fmax_rule)
        + ' WORK_DIR="{}"'.format(os.path.realpath(i_arch.tmp_dir))
        + ' SCRIPT_DIR="{}"'.format(os.path.realpath(os.path.join(i_arch.tmp_dir, work_script_path)))
        + ' LOG_DIR="{}"'.format(os.path.realpath(os.path.join(i_arch.tmp_dir, log_path)))
        + ' CLOCK_SIGNAL="{}"'.format(i_arch.clock_signal)
        + ' TOP_LEVEL_MODULE="{}"'.format(i_arch.top_level_module)
        + ' LIB_NAME="{}"'.format(i_arch.lib_name)
        + " --no-print-directory",
      ...
      running_arch_list.append(Running_arch(process=process, target=i_arch.target, arch=i_arch.arch_name, display_name=i_arch.arch_display_name, status_file=fmax_status_file, progress_file=synth_status_file, tmp_dir=i_arch.tmp_dir))
