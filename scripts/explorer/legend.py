...
def create_legend_item(architecture, line_style, color, page_name=""):
    ...
    ], id=f'legend-item-{architecture}-{page_name}', style={'display': 'block', 'margin-bottom': '5px'})
...
def setup_callbacks(explorer, page_name):
    ...
    def update_legend_visibility(selected_target, selected_yaml):
        ...
        architectures_for_target = explorer.dfs[selected_yaml][explorer.dfs[selected_yaml]['Target'] == selected_target]['Architecture'].unique()
