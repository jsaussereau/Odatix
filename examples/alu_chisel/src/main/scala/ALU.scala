...
class ALU(BITS: Int) extends Module {
  ...
  when (io.i_rst) {
    ...
    switch(io.i_sel_op) {
      ...
      is (ALUOp.alu_sll)  { value := io.i_op_a << io.i_op_b(2, 0) }
      is (ALUOp.alu_srl)  { value := io.i_op_a >> io.i_op_b(2, 0) }
      is (ALUOp.alu_sra)  { value := (io.i_op_a.asSInt >> io.i_op_b(2, 0)).asUInt }
