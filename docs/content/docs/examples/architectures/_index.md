---
title: "Architectures"
description: "Seven example designs shipped with Odatix — from a four-language counter to a C++ filter turned into RTL by high-level synthesis — each isolating one mechanism of architecture exploration."
weight: 1
---

# Architecture Examples

An [architecture](/docs/features/architecture-exploration/) is a design plus the recipe that turns it into a family of configurations: where its sources live, which top level to implement, and which parameters to sweep. The seven examples below are all shipped in a fresh workspace, and all of them run out of the box once an EDA tool is installed.
...
- **Four RTL languages, plus Chisel and C++** — Verilog, SystemVerilog, VHDL, Scala and a design that only becomes RTL when a generator or a high-level synthesis is run.
...
{{< doc-card title="FIR filter (C++ / Vitis HLS)" link="/docs/examples/architectures/hls/" icon="sliders" accent="#ea580c" cta="Read the example" >}}
A filter written in C++ and turned into RTL by Vitis HLS, sweeping two HLS pragmas and a generation parameter before the design is synthesized like any other.
{{< /doc-card >}}
{{< /doc-cards >}}
...
| [FIR filter (HLS)](/docs/examples/architectures/hls/) | C++ | 3 variables | RTL generated by high-level synthesis, pragmas swept from the command line |
