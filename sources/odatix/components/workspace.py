...
def load_arch_selection_settings(path: str) -> dict:
    settings = DEFAULT_ARCH_SELECTION_SETTINGS.copy()
    if not path or not os.path.isfile(path):
        return settings
    try:
        with open(path, "r") as f:
            data = yaml.safe_load(f) or {}
        settings["overwrite"] = _parse_bool(data.get("overwrite"), settings["overwrite"])
        settings["ask_continue"] = _parse_bool(data.get("ask_continue"), settings["ask_continue"])
        settings["exit_when_done"] = _parse_bool(data.get("exit_when_done"), settings["exit_when_done"])
        settings["log_size_limit"] = int(data.get("log_size_limit", settings["log_size_limit"]))
        settings["nb_jobs"] = int(data.get("nb_jobs", settings["nb_jobs"]))
        settings["architectures"] = data.get("architectures", {})
    except Exception:
        return settings
    return settings

def save_architecture_selection(path, settings) -> None:
    """
    ...
    data.yaml_set_start_comment(
f"""##############################################
# Odatix settings for fmax synthesis
##############################################
